...
export async function sendVerificationEmail(to: string, code: string, type: 'register' | 'reset_password' | 'verify_email'): Promise<boolean> {
  try {
    const subject = type === 'register' ? '【y悠悠】注册验证码' : type === 'reset_password' ? '【y悠悠】密码重置验证码' : '【y悠悠】邮箱验证码';
    const action = type === 'register' ? '注册账号' : type === 'reset_password' ? '重置密码' : '验证邮箱';
