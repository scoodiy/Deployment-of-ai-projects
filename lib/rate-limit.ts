...
export const RATE_LIMITS = {
  ...
  commentIp: {
    maxRequests: 5,
    windowSeconds: 60,
    keyPrefix: 'rl:comment:ip',
    errorMessage: '评论过于频繁，请稍后再试',
  },
  commentDuplicate: {
    maxRequests: 1,
    windowSeconds: 300,
    keyPrefix: 'rl:comment:dup',
    errorMessage: '请勿重复发送相同内容',
  },
} as const;
...
const failureStore = new Map<string, { failures: number; until: number }>();

export function trackFailure(key: string, windowMs: number, maxFailures: number): boolean {
  const now = Date.now();
  let entry = failureStore.get(key);
  if (!entry || entry.until <= now) {
    entry = { failures: 1, until: now + windowMs };
...
export function getFailureBlockedUntil(key: string): number | null {
  const entry = failureStore.get(key);
  if (entry && entry.failures > 2 && entry.until > Date.now()) {
    return entry.until;
  }
  return null;
}
