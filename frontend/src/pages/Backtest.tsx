      ...
      {detail && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="card text-center">
            ...
            <div className={`text-lg font-bold ${(detail.total_return ?? 0) >= 0 ? 'text-rise' : 'text-fall'}`}>
              {detail.total_return != null ? `${detail.total_return > 0 ? '+' : ''}${detail.total_return.toFixed(2)}%` : '-'}
            </div>
          </div>
          <div className="card text-center">
            <div className="text-xs text-gray-500">年化收益</div>
            <div className={`text-lg font-bold ${(detail.annual_return ?? 0) >= 0 ? 'text-rise' : 'text-fall'}`}>
          ...
          <div className="card text-center">
            ...
            <div className="text-lg font-bold text-fall">{detail.max_drawdown != null ? `-${detail.max_drawdown.toFixed(2)}%` : '-'}</div>
          </div>
          <div className="card text-center">
            <div className="text-xs text-gray-500">夏普比率</div>
            <div className="text-lg font-bold">{detail.sharpe_ratio?.toFixed(2) ?? '-'}</div>
          </div>
          <div className="card text-center">
            <div className="text-xs text-gray-500">胜率</div>
            <div className="text-lg font-bold">{detail.win_rate != null ? `${detail.win_rate.toFixed(1)}%` : '-'}</div>
          </div>
        </div>
      )}
            ...
            <tbody className="divide-y divide-gray-100">
              {trades.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50">
                  <td className="px-3 py-2 text-gray-500">{t.trade_date}</td>
                  <td className="px-3 py-2 font-mono text-blue-600">{t.code}</td>
                  <td className={`px-3 py-2 font-medium ${t.direction === 'buy' ? 'text-rise' : 'text-fall'}`}>
                  ...
                  <td className="px-3 py-2 text-gray-500 max-w-[150px] truncate">{t.reason || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
