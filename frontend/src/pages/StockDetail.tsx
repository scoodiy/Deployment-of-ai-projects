  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/stocks" className="text-blue-600 hover:underline text-sm py-2 inline-block min-h-[36px]">← 返回列表</Link>
      ...
      {kline.length > 0 && (
        <div className="card">
          <h3 className="font-semibold mb-3">K线图（近{kline.length}日）</h3>
      ...
      {kline.length > 0 && (
        <div className="card overflow-x-auto">
          <h3 className="font-semibold mb-3">K线数据明细</h3>
          <table className="w-full text-sm font-mono">
            <thead>
              <tr className="table-header">
                <th className="px-2 md:px-3 py-2">日期</th>
                <th className="px-2 md:px-3 py-2 text-right hidden sm:table-cell">开盘</th>
                <th className="px-2 md:px-3 py-2 text-right hidden md:table-cell">最高</th>
                <th className="px-2 md:px-3 py-2 text-right hidden md:table-cell">最低</th>
                <th className="px-2 md:px-3 py-2 text-right">收盘</th>
                <th className="px-2 md:px-3 py-2 text-right">涨跌</th>
                <th className="px-2 md:px-3 py-2 text-right hidden sm:table-cell">成交量</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {kline.slice(-20).reverse().map((k) => (
                <tr key={k.trade_date}>
                  <td className="px-2 md:px-3 py-1.5">{k.trade_date}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right hidden sm:table-cell">{k.open.toFixed(2)}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right text-rise hidden md:table-cell">{k.high.toFixed(2)}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right text-fall hidden md:table-cell">{k.low.toFixed(2)}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right">{k.close.toFixed(2)}</td>
                  <td className={`px-2 md:px-3 py-1.5 text-right ${(k.pct_change ?? 0) > 0 ? 'text-rise' : (k.pct_change ?? 0) < 0 ? 'text-fall' : ''}`}>
                    {k.pct_change != null ? `${k.pct_change > 0 ? '+' : ''}${k.pct_change.toFixed(2)}%` : '-'}
                  </td>
                  <td className="px-2 md:px-3 py-1.5 text-right hidden sm:table-cell">{k.volume.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {indicators.length > 0 && (
        <div className="card">
          ...
          <IndicatorChart kline={kline} indicators={indicators} />
        </div>
      )}
      ...
      {indicators.length > 0 && (
        <div className="card overflow-x-auto">
          <h3 className="font-semibold mb-3">指标明细</h3>
          <table className="w-full text-sm font-mono">
            <thead>
              <tr className="table-header">
                <th className="px-2 md:px-3 py-2">日期</th>
                <th className="px-2 md:px-3 py-2 text-right">MA5</th>
                <th className="px-2 md:px-3 py-2 text-right hidden sm:table-cell">MA20</th>
                <th className="px-2 md:px-3 py-2 text-right">MACD</th>
                <th className="px-2 md:px-3 py-2 text-right hidden sm:table-cell">KDJ-K</th>
                <th className="px-2 md:px-3 py-2 text-right hidden md:table-cell">RSI6</th>
                <th className="px-2 md:px-3 py-2 text-right hidden md:table-cell">BOLL上</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {indicators.slice(-20).reverse().map((i) => (
                <tr key={i.trade_date}>
                  <td className="px-2 md:px-3 py-1.5">{i.trade_date}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right">{i.ma5?.toFixed(2) ?? '-'}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right hidden sm:table-cell">{i.ma20?.toFixed(2) ?? '-'}</td>
                  <td className={`px-2 md:px-3 py-1.5 text-right ${(i.macd_hist ?? 0) > 0 ? 'text-rise' : 'text-fall'}`}>{i.macd_hist?.toFixed(3) ?? '-'}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right hidden sm:table-cell">{i.kdj_k?.toFixed(1) ?? '-'}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right hidden md:table-cell">{i.rsi6?.toFixed(1) ?? '-'}</td>
                  <td className="px-2 md:px-3 py-1.5 text-right hidden md:table-cell">{i.boll_upper?.toFixed(2) ?? '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
