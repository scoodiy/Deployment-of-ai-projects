            ...
            <tbody className="divide-y divide-gray-100">
              {items.map((w) => (
                <tr key={w.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-blue-600 cursor-pointer" onClick={() => navigate(`/stocks/${w.code}`)}>{w.code}</td>
                  <td className="px-4 py-3">{w.name || '-'}</td>
                  <td className="px-4 py-3 text-right font-mono">{w.price?.toFixed(2) ?? '-'}</td>
                  <td className={`px-4 py-3 text-right font-mono ${(w.pct_change ?? 0) > 0 ? 'text-rise' : (w.pct_change ?? 0) < 0 ? 'text-fall' : ''}`}>
                    {w.pct_change != null ? `${w.pct_change > 0 ? '+' : ''}${w.pct_change.toFixed(2)}%` : '-'}
                  </td>
                  <td className="px-4 py-3 text-right">{w.amount != null ? (w.amount / 1e8).toFixed(2) + '亿' : '-'}</td>
                  <td className="px-4 py-3 text-gray-500">{w.remark || '-'}</td>
                  <td className="px-4 py-3">
                    <button className="text-red-500 hover:underline text-sm" onClick={() => handleRemove(w.code)}>取消关注</button>
                  </td>
                </tr>
              ))}
            </tbody>
