...
export default function KlineChart({ data, height }: Props) {
  const chartRef = useRef<HTMLDivElement>(null)
  const instanceRef = useRef<echarts.ECharts | null>(null)
  const { width } = useWindowSize()

  const resolvedHeight = height ?? (width < 640 ? 300 : width < 768 ? 350 : 500)
  ...
  useEffect(() => {
    ...
    const sliderBottom = isMobile ? 5 : 10
  ...
  return <div ref={chartRef} style={{ width: '100%', height: resolvedHeight }} />
}
