...
def _retry_request(client: httpx.Client, url: str, max_retries: int = MAX_RETRIES):
    """带指数退避重试的 HTTP 请求。"""
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning("请求重试 %d/%d: %s, 等待 %.1fs", attempt + 1, max_retries, e, delay)
                time.sleep(delay)
            else:
                raise
...
def fetch_stock_list() -> list[dict]:
    ...
    with httpx.Client(headers=TENCENT_HEADERS, timeout=30) as client:
        for i in range(0, len(all_codes), BATCH_SIZE):
            ...
            try:
                resp = _retry_request(client, f"{TENCENT_API}{codes_str}")
                ...
                if batch_num % 20 == 0:
                    logger.info(
                        "进度: %d/%d 批次, 已获取 %d 只股票",
                        batch_num, total_batches, len(all_stocks),
                    )

            except Exception as e:
                consecutive_failures += 1
                logger.warning("批次 %d 失败: %s (连续失败 %d 次)", batch_num, e, consecutive_failures)
                if consecutive_failures >= 5:
                    logger.error("连续 %d 批次失败，停止抓取", consecutive_failures)
                    break
    ...
    logger.info("共获取 %d 只有效股票", len(all_stocks))
    return all_stocks
...
def save_stock_list(db: Session, stocks: list[dict]) -> dict:
    ...
    logger.info("保存完成: 新增 %d, 更新 %d", inserted, updated)
    return result


def run_full_crawl(db: Session) -> dict:
    """执行完整数据抓取。"""
    logger.info("开始抓取 A 股实时行情（腾讯数据源）...")
