...
@router.post("", response_model=ChatResponse, summary="股票知识问答")
        ...
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                if resp.status_code != 200:
                    ...
                    logger.warning("LLM API 返回格式异常: %s", str(data)[:500])
        except httpx.TimeoutException:
            logger.warning("LLM API 调用超时，回退到内置回答")
        except httpx.HTTPStatusError as e:
            logger.warning("LLM API HTTP 错误: %s", e)
        except Exception as e:
            logger.warning("LLM API 调用失败，回退到内置回答: %s", str(e))
