...
export default function ClientSocials() {
  ...
  return (
    <div className="flex gap-2 flex-wrap justify-center mt-4">
      <SocialBtn type="github" url={config.github_url} />
      <SocialBtn type="gitee" url={config.bilibili_url} />
      <SocialBtn type="email" onClick={() => copyToClipboard(config.email || '', '邮箱')} />
      <SocialBtn type="qq" onClick={() => copyToClipboard(config.qq || '', 'QQ号')} />
      <SocialBtn type="wechat" onClick={() => copyToClipboard(config.wechat || '', '微信号')} />
    </div>
  );
}