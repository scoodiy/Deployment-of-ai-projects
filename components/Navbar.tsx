...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <header className={`w-full fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b ${showNav ? 'translate-y-0' : '-translate-y-full'} bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border-white/20 dark:border-white/5 shadow-sm`}>
        <div className="w-[90%] max-w-6xl mx-auto h-16 flex items-center justify-between px-4 sm:px-[30px] box-border">
          ...
          <button
            ...
            className="lg:hidden w-10 h-10 rounded-xl bg-white/50 dark:bg-slate-800/50 flex items-center justify-center text-slate-700 dark:text-slate-300 hover:bg-indigo-500 hover:text-white transition-all duration-300 border border-white/40 dark:border-white/10 shadow-sm"
          >
      ...
      {/* 📱 手机端：可拖拽吸附的触发球（汉堡按钮已移入header，这里保留轮盘菜单） */}
              ...
              >
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      transform: `rotate(${mobileNavLinks.length * (360 / mobileNavLinks.length)}deg) translateY(-65px) rotate(-${mobileNavLinks.length * (360 / mobileNavLinks.length)}deg)`
                    }}
                  >
                    <motion.div style={{ rotate: inverseRotation }} className="w-full h-full">
                      {user ? (
                        <Link
                          href="/user/settings"
                          onClick={() => setIsMobileMenuOpen(false)}
                          className="flex flex-col items-center justify-center w-full h-full rounded-full bg-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.6)] hover:scale-110 transition-all"
                          title="个人设置"
                        >
                        ...
                        <Link
                          href="/login"
                          onClick={() => setIsMobileMenuOpen(false)}
                          className="flex flex-col items-center justify-center w-full h-full rounded-full bg-white/90 dark:bg-slate-800 text-indigo-500 shadow-md border border-white/50 dark:border-slate-600 hover:scale-110 transition-all"
                        >
                          <span className="text-[10px] font-black">登录</span>
                          <span className="text-[7px] mt-0.5">注册</span>
                        </Link>
                      )}
                    </motion.div>
                  </div>
