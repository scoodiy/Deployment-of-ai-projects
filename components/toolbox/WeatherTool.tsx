...
import { Cloud, Sun, CloudRain, Snowflake, Loader2, Wind, Droplets, Eye, MapPin } from 'lucide-react';
    ...
    >
      ...
      <div className="flex gap-1.5">
        <input
          ...
          placeholder="输入城市名"
          className="flex-1 px-3 py-2.5 text-sm rounded-xl bg-white/60 dark:bg-slate-700/60 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:border-indigo-400 dark:focus:border-indigo-500 transition-colors"
        />
        <button
          onClick={handleLocate}
          disabled={locating}
          className="min-w-[44px] min-h-[44px] flex items-center justify-center bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 disabled:opacity-40 transition-all active:scale-95 shadow-sm"
          title="定位当前位置"
        >
        ...
        <button
          ...
          disabled={loading}
          className="min-w-[44px] min-h-[44px] px-3 bg-indigo-500 text-white text-sm font-bold rounded-xl hover:bg-indigo-600 disabled:opacity-40 transition-all active:scale-95 shadow-sm"
        >
          {loading ? '...' : '查'}
        </button>
      </div>
