    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col items-center gap-4">
      ...
      <div className="flex gap-2">
        <button onClick={toggle}
          className="px-4 min-h-[44px] bg-indigo-500 text-white text-sm font-bold rounded-xl hover:bg-indigo-600 transition-all active:scale-95">
          {phase === 'idle' ? '开始' : '暂停'}
        </button>
        <button onClick={reset}
          className="px-4 min-h-[44px] bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 text-sm font-bold rounded-xl hover:bg-slate-300 dark:hover:bg-slate-500 transition-all active:scale-95">
          重置
        </button>
      </div>
    </motion.div>
