    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col items-center gap-3">
      ...
      <div className="grid grid-cols-4 gap-1.5 p-2 bg-slate-200 dark:bg-slate-700 rounded-xl">
        {board.flat().map((val, i) => (
          <div key={i} className={`w-14 h-14 sm:w-16 sm:h-16 rounded-lg flex items-center justify-center text-sm font-black ${COLORS[val] || 'bg-red-600 text-white'}`}>
            {val || ''}
          </div>
        ))}
      </div>
      <div className="flex flex-col items-center gap-1 sm:hidden">
        <button onClick={() => move('up')} className="w-12 h-8 bg-slate-200 dark:bg-slate-700 rounded-lg text-xs font-bold">↑</button>
        <div className="flex gap-1">
          <button onClick={() => move('left')} className="w-12 h-8 bg-slate-200 dark:bg-slate-700 rounded-lg text-xs font-bold">←</button>
          <button onClick={() => move('down')} className="w-12 h-8 bg-slate-200 dark:bg-slate-700 rounded-lg text-xs font-bold">↓</button>
          <button onClick={() => move('right')} className="w-12 h-8 bg-slate-200 dark:bg-slate-700 rounded-lg text-xs font-bold">→</button>
        </div>
      </div>
