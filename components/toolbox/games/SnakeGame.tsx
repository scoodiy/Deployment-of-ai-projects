  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col items-center gap-3">
      ...
      <div className="flex flex-col items-center gap-1.5 sm:hidden">
        <button onClick={() => { dirRef.current = { x: 0, y: -1 }; }} className="w-14 h-10 bg-slate-300 dark:bg-slate-600 rounded-lg text-sm font-bold active:bg-indigo-400 active:text-white">↑</button>
        <div className="flex gap-1.5">
          <button onClick={() => { dirRef.current = { x: -1, y: 0 }; }} className="w-14 h-10 bg-slate-300 dark:bg-slate-600 rounded-lg text-sm font-bold active:bg-indigo-400 active:text-white">←</button>
          <button onClick={() => { dirRef.current = { x: 0, y: 1 }; }} className="w-14 h-10 bg-slate-300 dark:bg-slate-600 rounded-lg text-sm font-bold active:bg-indigo-400 active:text-white">↓</button>
          <button onClick={() => { dirRef.current = { x: 1, y: 0 }; }} className="w-14 h-10 bg-slate-300 dark:bg-slate-600 rounded-lg text-sm font-bold active:bg-indigo-400 active:text-white">→</button>
        </div>
      </div>
