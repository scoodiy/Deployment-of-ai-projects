...
const NUM_COLORS = ['', 'text-blue-500', 'text-green-600', 'text-red-500', 'text-purple-600', 'text-yellow-600', 'text-cyan-500', 'text-slate-500', 'text-slate-400'];
      ...
      <div className="inline-grid gap-0.5" style={{ gridTemplateColumns: `repeat(${COLS}, 1fr)` }}>
        {board.map((row, r) => row.map((cell, c) => (
          <button key={`${r}-${c}`} onClick={() => reveal(r, c)} onContextMenu={e => flag(e, r, c)}
            className={`w-8 h-8 sm:w-9 sm:h-9 rounded-md text-xs font-bold flex items-center justify-center transition-all ${
              cell.revealed
