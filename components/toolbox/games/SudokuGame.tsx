...
function isValid(board: number[][], r: number, c: number, num: number) {
  for (let i = 0; i < 9; i++) {
        ...
        {board.map((row, r) => row.map((val, c) => {
          ...
          return (
            <button key={`${r}-${c}`}
              ...
              className={`w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-xs font-bold transition-all ${boxBorder} ${boxBorderC}
        ...
        <div className="flex gap-1 flex-wrap justify-center">
          {[1,2,3,4,5,6,7,8,9].map(n => (
            <button key={n} onClick={() => setCell(selected[0], selected[1], n)}
              className="w-8 h-8 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 rounded-lg text-xs font-bold hover:bg-indigo-200 dark:hover:bg-indigo-800 active:scale-95">{n}</button>
          ))}
          <button onClick={() => setCell(selected[0], selected[1], 0)}
            className="w-8 h-8 bg-slate-200 dark:bg-slate-600 text-slate-500 rounded-lg text-xs font-bold hover:bg-slate-300 dark:hover:bg-slate-500">✕</button>
        </div>
      )}
