    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col gap-3">
      <div className="flex gap-1.5">
        <input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === 'Enter' && generate()}
          placeholder="输入文本或链接..." className="flex-1 px-3 py-2.5 text-sm rounded-xl bg-white/60 dark:bg-slate-700/60 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:border-indigo-400" />
        <button onClick={generate} disabled={!text.trim()}
          className="min-w-[44px] min-h-[44px] px-4 bg-indigo-500 text-white text-sm font-bold rounded-xl hover:bg-indigo-600 disabled:opacity-40 transition-all active:scale-95">生成</button>
      </div>
      {qrUrl && (
