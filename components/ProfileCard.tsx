    ...
    >
      <div className="flex items-start justify-between relative z-10">
        <div className="flex items-center gap-6">
          <div className="w-24 h-24 rounded-2xl bg-gradient-to-tr from-indigo-500 to-purple-500 p-1 shadow-lg flex-shrink-0 transition-transform duration-500 group-hover:rotate-3">
            <img src={siteConfig.avatar_url || '/default-avatar.png'} alt="avatar" className="w-full h-full rounded-xl object-cover bg-white" />
          </div>
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2 tracking-wider transition-colors duration-700">{siteConfig.site_title}</h1>
      ...
      <div className="flex flex-col md:flex-row items-end md:items-center justify-between mt-8 gap-6 relative z-10">
        ...
        <div className="flex gap-3 flex-wrap justify-end" onClick={(e) => e.stopPropagation()}>
          <SocialBtn type="github" url={siteConfig.github_url} />
          <SocialBtn type="gitee" url={siteConfig.bilibili_url} />
          <SocialBtn type="email" onClick={() => copyToClipboard(siteConfig.email || '', '邮箱')} />
          <SocialBtn type="qq" onClick={() => copyToClipboard(siteConfig.qq || '', 'QQ号')} />
          <SocialBtn type="wechat" onClick={() => copyToClipboard(siteConfig.wechat || '', '微信号')} />
        </div>
      </div>
    </div>
  );
