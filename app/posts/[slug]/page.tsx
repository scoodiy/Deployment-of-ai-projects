...
export async function generateStaticParams() {
  try {
    const db = getDb();
    const blogs = db.prepare("SELECT slug FROM blogs WHERE status = 'published'").all() as BlogSlugRow[];
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const blog = db.prepare(
    "SELECT title, summary, cover_image, published_at FROM blogs WHERE slug = ? AND status = 'published'"
  ).get(slug) as BlogMetaRow | undefined;

  if (!blog) {
    return { title: '文章不存在 - y悠悠' };
  }

  const title = `${blog.title} - y悠悠`;
  const description = blog.summary || '';
  const url = `https://ayuu.fun/posts/${slug}`;
...
async function getPostData(slug: string) {
  ...
  ).get(slug) as BlogDetailRow | undefined;
...
function getRecentPosts(currentSlug: string) {
  try {
    ...
    ).all(currentSlug) as RecentPostRow[];
    return blogs;
  } catch (_e) {
    return [];
  }
    ...
    <div className="min-h-screen relative pb-20">
      ...
      <PageTransition>
        <main className="w-[95%] md:w-[90%] max-w-6xl mx-auto mt-24 md:mt-28 flex flex-col lg:flex-row gap-6 md:gap-8 relative z-10">
          ...
          <article className="flex-1 bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 dark:border-white/10 overflow-hidden transition-colors duration-700">
            <div className="w-full aspect-video bg-slate-200 dark:bg-slate-700 relative group">
              <img src={postData.cover} alt="封面" loading="lazy" className="w-full h-full object-cover opacity-90 transition-transform duration-1000 group-hover:scale-105" />
            </div>
            ...
            <div className="p-5 md:p-12 relative">
              ...
              <header className="mb-6 md:mb-8 border-b border-slate-300/50 dark:border-slate-700 pb-5 md:pb-6 relative">
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                  <div className="flex items-center gap-1.5 md:gap-2 text-indigo-700 dark:text-indigo-400 font-bold bg-white/30 dark:bg-slate-900/30 px-3 md:px-4 py-1.5 md:py-2 rounded-full w-max text-xs md:text-sm transition-colors duration-700 shadow-sm border border-white/20 dark:border-white/5">
                  ...
                  {postData.tags.map((tag: string) => (
                    <div key={tag} className="flex items-center gap-1 text-pink-600 dark:text-pink-400 font-bold bg-white/30 dark:bg-slate-900/30 px-2.5 md:px-3 py-1.5 md:py-2 rounded-full text-xs md:text-sm transition-colors duration-700 shadow-sm border border-white/20 dark:border-white/5">
                      # {tag}
                    </div>
                  ))}
                </div>
          ...
          <aside className="w-full lg:w-72 xl:w-80 flex-shrink-0 space-y-6">
            <div className="sticky top-24">
              ...
              {recentPosts.length > 0 && (
                <div className="mt-6 bg-white/40 dark:bg-slate-800/30 backdrop-blur-xl rounded-2xl p-5 shadow-lg border border-white/40 dark:border-white/10">
