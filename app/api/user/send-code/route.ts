...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validTypes = ['register', 'reset_password', 'verify_email'];
    ...
    // For password reset and email verification, check if email exists
    if (codeType === 'reset_password' || codeType === 'verify_email') {
      const existing = db.prepare('SELECT id FROM users WHERE email = ?').get(email);
      if (!existing) {
