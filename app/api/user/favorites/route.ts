...
export async function GET(request: Request) {
  ...
  ).all(Number(user.userId), targetType);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = idempotentFavorite(Number(user.userId), target_type, target_id);
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const result = idempotentUnfavorite(Number(user.userId), target_type, target_id);
    return NextResponse.json(result);
  } catch (error) {
    if (process.env.NODE_ENV === 'development') {
