...
export const GET = withAdminAuth(async (request) => {
  ...
  const db = getDb();
  let query = 'SELECT id, title, slug, summary, cover_image, category, tags, status, view_count, created_at, published_at FROM blogs';
...
export const POST = withAdminAuth(async (request, admin) => {
  ...
  const result = db.prepare(`
    INSERT INTO blogs (title, slug, summary, content, cover_image, category, tags, status, published_at, seo_title, seo_description, og_image)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(title, slug, summary || '', content || '', cover_image || '', category || '', tagsStr, status || 'draft', publishedAt, seo_title || '', seo_description || '', og_image || '');

  db.prepare('INSERT INTO operation_logs (admin_id, action, target_type, target_id, detail, ip) VALUES (?, ?, ?, ?, ?, ?)').run(
    Number(admin.adminId), 'create', 'blog', result.lastInsertRowid, `创建博客: ${title}`, getIp(request)
  );
