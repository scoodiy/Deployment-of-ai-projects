...
export const PUT = withAdminAuth<{ params: Promise<{ id: string }> }>(async (request, admin, { params }) => {
  ...
  try {
    ...
    // 不能修改自己的角色
    if (Number(id) === Number(admin.adminId)) {
    ...
    db.prepare('INSERT INTO admin_operation_logs (admin_id, action, target_type, target_id, detail, ip) VALUES (?, ?, ?, ?, ?, ?)').run(
      Number(admin.adminId), 'change_role', 'user', id, `修改用户角色: ${user.username} -> ${role}`, request.headers.get('x-forwarded-for') || 'unknown'
    );

    return NextResponse.json({ success: true, message: `用户角色已更新为 ${role}` });
  } catch (error) {
    return safeError('修改失败', 500, error);
  }
});