          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => { setCat(c); setActiveId(null); }}
                className={`text-xs font-bold px-4 py-2 rounded-full transition-all ${cat === c ? 'bg-indigo-500 text-white shadow-md' : 'bg-white/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 border border-white/40 dark:border-white/10'}`}>
                {c}
              </button>
            ))}
          </div>
                ...
                <motion.button key={tool.id}
                  ...
                  className={`text-left p-4 rounded-2xl border transition-all duration-300 ${
                    isActive
                      ...
                      : 'bg-white/50 dark:bg-slate-800/50 backdrop-blur-md border-white/40 dark:border-white/10 shadow-lg hover:shadow-xl hover:border-indigo-300 dark:hover:border-indigo-700 hover:scale-[1.02]'
                  }`}>
          ...
          <AnimatePresence mode="wait">
            {activeTool && (
              <motion.div key={activeTool.id}
                ...
                <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-2xl border border-white/40 dark:border-white/10 shadow-2xl rounded-3xl p-6">
