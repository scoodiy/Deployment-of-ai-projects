...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    const title = `${data.title || '碎片记录'} - y悠悠`;
    const description = typeof data.description === 'string' ? data.description : (data.title || '碎片记录');
    const url = `https://ayuu.fun/chatter/${slug}`;
    ...
    return {
      ...
      openGraph: {
        ...
        siteName: 'y悠悠',
        locale: 'zh_CN',
        publishedTime: data.date instanceof Date ? data.date.toISOString() : undefined,
    ...
    <div className="min-h-screen relative pb-20">
      ...
      <PageTransition>
        <main className="w-[95%] md:w-[90%] max-w-6xl mx-auto mt-24 md:mt-28 flex flex-col lg:flex-row gap-6 md:gap-8 relative z-10">
          ...
          <article className="flex-1 bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl rounded-[40px] shadow-2xl border border-white/40 dark:border-white/10 overflow-hidden transition-colors duration-700">
            {chatterData.cover && (
              <div className="w-full aspect-video bg-slate-200 dark:bg-slate-700 relative group">
                <img src={chatterData.cover} alt="封面" loading="lazy" className="w-full h-full object-cover opacity-90 transition-transform duration-1000 group-hover:scale-105" />
              </div>
            )}
          ...
          <aside className="w-full lg:w-[320px] flex flex-col gap-6 flex-shrink-0">
            <div className="bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl rounded-3xl p-6 border border-white/40 dark:border-white/10 shadow-xl text-center">
            ...
            <div className="bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl rounded-3xl p-6 border border-white/40 dark:border-white/10 shadow-xl">
