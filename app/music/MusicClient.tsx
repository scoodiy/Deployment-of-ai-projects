...
export default function MusicClient() {
  ...
  const parsedLyrics = useMemo<LyricLine[]>(() => {
    ...
    return (lines.map(l => ({ time: -1, text: l.trim() })) as LyricLine[]).filter(l => l.text);
  }, [currentSong]);
      ...
      <div className="min-h-screen relative pb-32 flex flex-col">
        ...
        <PageTransition>
          <div className="w-full max-w-7xl mx-auto mt-24 md:mt-28 px-4 sm:px-6 md:px-10 relative z-10">
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-4 bg-white/40 dark:bg-slate-800/30 backdrop-blur-md border border-white/40 dark:border-white/10 rounded-[32px] shadow-2xl">
      ...
      <PageTransition>
        <div className="w-full max-w-7xl mx-auto mt-24 md:mt-28 px-4 sm:px-6 md:px-10 relative z-10">
          ...
          <div className="flex flex-col md:grid md:grid-cols-12 gap-6 md:gap-8 w-full md:items-stretch md:h-[calc(100vh-320px)] md:min-h-[600px] md:max-h-[720px]">
            ...
            {/* ====== 左侧/顶部：播放控制台 ====== */}
            <div className="md:col-span-5 flex flex-col bg-white/40 dark:bg-slate-800/30 backdrop-blur-md border border-white/40 dark:border-white/10 rounded-[32px] shadow-2xl p-6 md:p-10 relative overflow-hidden transition-all duration-500 shrink-0 min-h-[460px] sm:min-h-[500px] md:min-h-0">
            ...
            {/* ====== 右侧/底部：歌词与歌单面板 ====== */}
            <div className="md:col-span-7 flex flex-col bg-white/40 dark:bg-slate-800/30 backdrop-blur-md border border-white/40 dark:border-white/10 rounded-[32px] shadow-2xl relative transition-colors duration-700 overflow-hidden min-h-[360px] h-[55vh] md:h-auto shrink-0">
              <div className="flex items-center justify-center gap-1 p-1 mt-4 md:mt-6 mx-auto bg-white/50 dark:bg-slate-900/30 rounded-full shadow-inner border border-white/40 w-48 md:w-64 z-20 shrink-0">
                              ...
                              parsedLyrics.map((line: LyricLine, index: number) => {
                                ...
                                return (
                                  <div key={index} ref={isActive ? activeLyricRef : null}
                                    ...
                                    onClick={() => duration > 0 && handleSeek({ target: { value: String((line.time / duration) * 100) } } as unknown as React.ChangeEvent<HTMLInputElement>)}
                                  >
          ...
          {/* ====== 留言板 ====== */}
          <div className="mt-8 md:mt-12 mb-20 bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl rounded-[32px] md:rounded-[40px] shadow-2xl border border-white/40 dark:border-white/10 overflow-hidden transition-colors duration-700 relative">
             <div className="px-5 sm:px-8 md:px-16 py-8 md:py-12 relative">
                ...
                <div className="relative"><SiteComments targetType="music" targetId={0} /></div>
             </div>
          </div>
        </div>
      </PageTransition>
