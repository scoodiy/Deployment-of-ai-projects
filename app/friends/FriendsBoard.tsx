...
export default function FriendsBoard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/friends')
      .then(res => res.json())
      .then(data => { if (!cancelled) { setFriendsData(data.friends || []); setLoading(false); } })
        ...
        {friendsData.map((friend) => (
          <motion.div key={friend.id} variants={itemVariants} className="h-full">
            <a
              ...
              className="block h-full rounded-2xl md:rounded-3xl bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-lg md:shadow-xl overflow-hidden transition-all duration-500 hover:-translate-y-1 md:hover:-translate-y-2 hover:scale-[1.02] group relative p-3 md:p-6"
            >
              {/* 卡片底部的动态光晕 */}
              <div
