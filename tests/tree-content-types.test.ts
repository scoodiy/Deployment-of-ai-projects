...
test('alchemy lab uses site-native comments instead of external Gitalk', async () => {
  const source = await readFile(alchemyUrl, 'utf8');

  assert.match(source, /type ContentItem = \{/);
  assert.match(source, /type Badge = \{/);
  // Gitalk 已移除，改用站内评论组件
  assert.match(source, /import LabComments/);
  assert.match(source, /<LabComments/);
  assert.doesNotMatch(source, /GitHubComment/);
  assert.doesNotMatch(source, /gitalk/);
});
